pipeline {
    ...
    stages{
        ...
        stage('deploy to stage') {
            steps{
                sshagent(['ansible-key']) {
                    sh 'ssh -t -t ec2-user@10.0.2.205 -o strictHostKeyChecking=no "cd /etc/ansible && ansible-playbook -i /etc/ansible/stage-hosts stage-env-playbook.yml"' 
               }
            }
        }
        ...
        stage('Deploy to prod') {
            steps{
               sshagent(['ansible-key']) {
                    sh 'ssh -t -t ec2-user@10.0.2.205 -o strictHostKeyChecking=no "cd /etc/ansible && ansible-playbook -i /etc/ansible/prod-hosts prod-env-playbook.yml"'
               }  
            }
        }
    }
